...
//////////////////
  private int getRangeIndex(InetAddress ip){
     int index = 0;
     Enumeration enum0 = hosts.elements();
     while(enum0.hasMoreElements()){
       InetRange ir = (InetRange) enum0.nextElement();
  ...
  private String userNames(int i){
    ...
    Enumeration enum0 = ((Hashtable)users.elementAt(i)).keys();
    if(!enum0.hasMoreElements()) return "";
    String s = enum0.nextElement().toString();
    while(enum0.hasMoreElements())
       s += "; "+enum0.nextElement();
